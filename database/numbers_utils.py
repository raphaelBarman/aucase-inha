...
def get_valid_seqs(seq, seq_size=4):
    ...
    return np.array([(c[0][0], c[-1][0])
                     for c in candidates[candidate_validity]]).astype(int)
...
def get_candidates(seq):
    ...
    split_idxs = sorted(
        (np.where(one_diff[1:] & ~one_diff[:-1])[0] + 1).tolist() +
        (np.where(~one_diff[1:] & one_diff[:-1])[0] + 2).tolist())
...
def expand_nums(nums, texts):
    ...
    num_ranges = num_ranges.combine_first(
        texts.str.extract('^([0-9]+)(?: ?- ?([0-9]+))?'))
    offset = 0
    valid_end_nums = num_ranges[num_ranges[1].notnull()]
    for idx, start, end in valid_end_nums.reset_index().values.astype(int):
        ...
        nums = np.insert(
            nums,
            idx + 1 + offset,
            np.array(list(zip([idx] * len(range_), range_))),
            axis=0)
        offset += len(range_)
    return nums
...
def complete_document(document):
    ...
    values = np.stack([
        completed_nums[:, 0], completed_nums[:, 1],
        [np.nan] * len(completed_nums)
    ]).T
    ...
    idx_group = df_completed_nums.groupby('idx')
    expanded_filter = (idx_group.first()['num'] != idx_group.last()['num'])
    expanded_values = idx_group.last()['num'][expanded_filter].values
    values = values[~df_completed_nums.index.duplicated(keep='first')]
    values[:, 2][np.where(expanded_filter)[0]] = expanded_values
    df_completed_nums = pd.DataFrame(
        values, columns=['idx', 'num', 'last_num_completed'])
    df_completed_nums['idx'] = df_completed_nums['idx'].astype(int)
    df_completed_nums.set_index('idx', inplace=True)

    return document.join(
        df_completed_nums,
        rsuffix='_completed').set_index(['doc', 'page', 'entity'])
