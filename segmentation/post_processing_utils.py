...
def get_bboxes_sel(file_name,
    ...
    bin_thresh = [bin_thresh_section, bin_thresh_content]
    ...
    if use_ocr:
        if use_inha_ocr:
            ...
            ocr_bboxes = get_google_ocr_bboxes(file_name, width, height, ocr_dir, idx2cote)
    ...
    for class_idx in range(2):
        ...
        bboxes_sel.extend(get_content_aware_bboxes(file_name,
                                               contours,
                                               gray_img,
                                               CLASSES[class_idx],
                                               area_thresh))
...
def get_bboxes_sel_fastrcnn(file_name,
    ...
    boxes_section = (pred_data.get('detection_boxes') * [h,w,h,w]).astype(int)[section_idx][probas_section > proba_threshold_section]
    bboxes_geom_section = predboxes2geom(boxes_section)

    probas_sale = pred_data.get('detection_scores')[sale_idx]
    boxes_sale = (pred_data.get('detection_boxes') * [h,w,h,w]).astype(int)[sale_idx][probas_sale > proba_threshold_sale]
    bboxes_geom_sale = predboxes2geom(boxes_sale)

    if (complete_with_lines_section or
        complete_with_bboxes_section or
        complete_with_lines_sale or
        complete_with_bboxes_sale) and use_ocr:
        if use_inha_ocr:
            ocr_bboxes = get_inha_ocr_bboxes(file_name, ocr_dir)
        else:
            height, width = gray_img.shape
    ...
    if use_ocr:
        if complete_with_lines_section and len(bboxes_geom_section) > 0:
            bboxes_geom_section = complete_geoms_with_geoms(bboxes_geom_section, lines_geom,line_padding_section, 0)
        if complete_with_bboxes_section and len(bboxes_geom_section) > 0:
            bboxes_geom_section = complete_geoms_with_geoms(bboxes_geom_section, ocr_bboxes_geom, 0, 0)

        if complete_with_lines_sale and len(bboxes_geom_sale) > 0:
            bboxes_geom_sale = complete_geoms_with_geoms(bboxes_geom_sale, lines_geom,line_padding_sale,0)
        if complete_with_bboxes_sale and len(bboxes_geom_sale) > 0:
            bboxes_geom_sale = complete_geoms_with_geoms(bboxes_geom_sale, ocr_bboxes_geom,0,0)
...
def filter_mask_by_area(mask, area):
    ...
    [cv2.drawContours(mask_filtered, [c], -1, 255, -1) for c in contours if cv2.contourArea(c) > area]
...
def complete_with_lines(mask, lines_geom, padding=5, copy=False):
    ...
    for contour in contours:
        ...
        for box in get_intersecting(contour, lines_geom):
            ...
            miny -= padding
            maxy += padding
            mask_copy = cv2.rectangle(mask, (minx, miny), (maxx, maxy), 255, -1)
    return mask

def cut_idents_with_context(mask, indent_threshold=0.1, area_threshold=200, crop_padding=10, copy=False):
    if copy:
        mask = mask.copy()
    cnt, contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, 2)
    ...
    for contour in contours:
        ...
        if w <= crop_padding*2:
            continue
        minx, miny, maxx, maxy = x+crop_padding, y, x+w-crop_padding, y+h
        ...
        mask[miny+top_crop+get_indent_lines(crop, indent_threshold)] = np.zeros(mask.shape[1])
    return mask
...
def cut_at_idents(mask, indent_lines, lines, padding=4, copy=False):
    ...
    to_remove = np.hstack([to_remove, np.array([(to_remove-i, to_remove+i) for i in range(padding)]).reshape(-1)])
    mask[to_remove] = np.zeros(mask.shape[1])
    return mask_copy

def get_content_aware_bboxes(file_name, contours, gray_img, class_, area_threshold=200):
    bboxes = []
    image = gray_img
    for contour in contours:
...
def get_lines_and_bboxes(bboxes, line_threshold=5):
    ...
    """
    diffs = np.abs((bboxes[:,:,1].mean(axis=1)[::-1][:-1]-bboxes[:,:,1].mean(axis=1)[::-1][1:]))
    diffs[diffs < line_threshold] = 0
    mask_diffs = diffs > 0
    lines = np.array([x.mean() for x in np.split(bboxes[:,:,1].mean(axis=1)[::-1], np.where(mask_diffs)[0]+1)]).astype(int)
    ...
    return np.array(sorted(np.stack([lines, start_xs, end_xs], axis=-1), key=lambda x: x[0])), np.split(bboxes[::-1], np.where(mask_diffs)[0]+1)[::-1]

## Misc

def get_indent_lines(mask, indent_threshold=0.1):
    mean_column = (mask//255).sum(axis=0)
    crop_left, crop_right = (mean_column > 0).argmax(), len(mean_column)-(mean_column > 0)[::-1].argmax()
    indent_lines = np.where((mask[:,crop_left:crop_right].argmax(axis=1)/(crop_right-crop_left)) > indent_threshold)[0]
...
def complete_geoms_with_geoms(geoms, other_geoms, padding_v=5, padding_h = 0):
    ...
    geoms_res = []
    for geom in geoms:
        geom_minx, geom_miny, geom_maxx, geom_maxy = np.array(geom.bounds).astype(int)
        for box in get_intersecting(geom, other_geoms):
            ...
            if maxy > geom_maxy:
                geom_maxy = maxy
        geoms_res.append(geometry.box(geom_minx, geom_miny, geom_maxx, geom_maxy))
    return geoms_res
...
def get_lines(bboxes, line_threshold=5):
    ...
    start_xs = np.array([x.min() for x in xs_splitted]).astype(int)
    end_xs = np.array([x.max() for x in xs_splitted]).astype(int)
    return np.array(sorted(np.stack([lines, start_xs, end_xs], axis=-1), key=lambda x: x[0]))
...
def get_inha_ocr_bboxes(file_name, ocr_dir = '/scratch/raphael/data/ocr/ocr_inha/'):
    filepath = os.path.join(ocr_dir, file_name.replace('.jpg', '') + '_ocr.json.gz')
    if not os.path.exists(filepath):
...
def get_not_intersecting_with_class(bboxes):
    ...
    description_bboxes = [box[1] for box in bboxes if box[0] == 'sale_description']
    ...
    if len(description_bboxes_sel) > 0:
        description_bboxes_sel = [('sale_description', x ) for x in description_bboxes_sel]
...
def alto2coords(alto):
    ...
    w = int(alto['WIDTH'])
    h = int(alto['HEIGHT'])
    return np.array([[x,y], [x+w, y], [x+w, y+h], [x, y+h]]).astype(int)
