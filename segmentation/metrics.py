...
def compute_ious(pred, gt):
    ious = []
    iou_matrix = np.array(np.split(np.array([compute_IoU(x1,x2) for x1, x2 in product(pred, gt)]), len(pred)))
    while True:
...
def per_class_mAP(pred, gt, range_start=0.5, range_end=0.95, range_step=0.05):
    ...
    if sum(gt_section_idx) == 0:
        ...
        mAP_section = mAP(pred_section, gt_pred, range_start, range_end, range_step)
    else:
        mAP_section = 0.0
